...
if page == "Dashboard":
    ...
    if not probability_curve.empty:
        ...
        m2.metric("Rental Hashrate", fmt_hashrate_from_ph(best.get("hashrate_ph", 0)))
        m3.metric("Mission Duration", f"{duration_hours:.2f} h")
        m4.metric("Final Success Chance", f"{recommended_probability_pct:.1f}%")
        m5.metric("Median Success Time", f"{median_hours:.2f} h")
        m6.metric("Expected Success Time", f"{expected_time_hours:.2f} h")
    ...
    if pool_name:
        ...
        if len(pool_rankings) > 1:
            ...
            for rank, pool in enumerate(pool_rankings[1:4], start=2):
                st.info(
                    f"#{rank} • {pool.get('pool_name', 'Unknown')}\n\n"
                    f"Routing Score: {safe_num(pool.get('routing_score', 0)):.1f}/100\n"
                    f"Dominance: {safe_num(pool.get('pool_dominance_pct', 0)):.1f}%"
                )
    ...
    if frontier:
            ...
            rows.append(
                {
                    ...
                    "cost_usd": safe_num(frontier_best.get("cost_usd", 0)),
        ...
        display_frontier_df["budget_usd"] = display_frontier_df[
            "budget_usd"
        ].map(lambda x: f"${x:,.0f}")
        display_frontier_df["cost_usd"] = display_frontier_df[
            "cost_usd"
        ].map(lambda x: f"${x:,.0f}")
        display_frontier_df["duration_hours"] = display_frontier_df[
            "duration_hours"
        ].map(lambda x: f"{x:.2f}h")
        display_frontier_df["prob_1plus_pct"] = display_frontier_df[
            "prob_1plus_pct"
        ].map(lambda x: f"{x:.2f}%")
        display_frontier_df["prob_2plus_pct"] = display_frontier_df[
            "prob_2plus_pct"
        ].map(lambda x: f"{x:.2f}%")
        display_frontier_df["fvr"] = display_frontier_df["fvr"].map(
            lambda x: f"{x:.3f}"
        )
        display_frontier_df["risk_roi_pct"] = display_frontier_df[
            "risk_roi_pct"
        ].map(lambda x: f"{x:.2f}%")
        ...
        with st.expander("View all candidate plans", expanded=False):
            ...
            chart_df["prob_1plus_pct"] = chart_df[
                "prob_1plus_pct"
            ].round(2)
